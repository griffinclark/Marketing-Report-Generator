...
def generate_test():
    ...
    res = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system", "content": "You are a data-driven marketer who specializes in building tests to validate hypotheses"},
            {"role": "user", "content": test_selection_prompt +
...
def figure_out_what_we_can_expect_to_learn():
    res = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system", "content": "You are a data-driven marketer who specializes in building tests to validate hypotheses"},
            {"role": "user", "content": "Based on my report (linked below), what can I expect to learn from the test that I've outlined? Explain how it will validate or invalidate my hypothesis \n\n"},
            {"role": "assistant", "content": example_expected_learning_response},
            {"role": "user",
...
def main(url, user_prompt, test_mode):
    # if test_mode == True:
    #     logging.info("Test mode enabled. Only using " + fast_model)
    #     test_mode = True
    # else:
    #     logging.info("Prod mode enabled. Using " + smart_model + " and " + fast_model)
    #     test_mode = False

    logging.info("Refining my theories...")
    # report_data[company_name_key] = get_company_name(fetch_website_text(url))
    logging.info("(1/7) Understanding what your company does...")
    ...
    report_data[company_description_key] = report_data[company_description_key].replace(
        '\n', ' ')
    # logging.info("(2/8) Understanding the problem...")
    # report_data[explain_problem_key] = describe_user_problem(user_prompt)
    logging.info("(2/7) Finding a KPI that makes sense...")
    report_data[kpi_used_to_measure_problem_key] = generate_kpi()
    logging.info("(3/7) Thinking up a hypothesis...")
    report_data[hypothesis_key] = generate_hypothesis(web_copy=fetch_website_text(url))
    logging.info("(4/7) Generating ideas to test the hypothesis...")
    report_data[recommended_test_key] = generate_test()
    logging.info("(5/7) Making sure the test fits with what we want to learn...")
    report_data[what_we_can_expect_to_learn_key] = figure_out_what_we_can_expect_to_learn()
    logging.info("(6/7) Writing a guide for performing this test...")
    report_data[how_to_perform_test_key] = how_to_perform_test()
    logging.info("(7/7) Prepping the report...")
...
# explain_problem_key = "Why Focus Here?"
...
example_kpi_selection_response = "I recommend prioritizing Bounce Rate. It's better than Page Views, Session Duration, and New Sessions because it directly gauges initial user engagement, which is crucial for conversions. While other KPIs show website activity, they lack context - visitors might browse many pages, stay longer, or be new without performing desired actions. Bounce Rate, however, allows us to see if users find our content immediately compelling, highlighting opportunities to improve and enhance user experience to boost conversions."
...
example_problem_response = "You're grappling with a Conversion Rate Optimization (CRO) issue: plenty of visitors land on your site, but none reach out. CRO is the process of increasing the percentage of users who perform a desired action on your website. It's better to enhance engagement, a cost-effective and precise solution, instead of high-cost, less focused alternatives like site overhauls, new marketing strategies, or ad buys. By improving your content and user experience, you address the conversion problem directly and efficiently."
example_hypothesis_response = "Addressing the specific challenge of client conversion, a solution could be personalizing the sales process with AI-driven analytics and consistent follow-ups post meeting. This approach will allow your company to be seen as a problem solver rather than just a service provider, building trust and improving conversion rates. The personalized follow-ups serve to keep your company's value proposition at the forefront of potential clients' minds, further increasing conversion chances."
...
example_test_selection_response = "I propose A/B testing on your Service Explanation with a technical and simplified version, comparing their conversion rates to validate or invalidate the need for a more accessible explanation. Alternative tests, such as Multivariate Testing and Usability Testing, are less suitable. Multivariate testing, although comprehensive, requires substantial traffic to produce reliable results, making it less efficient for your case. Usability Testing, focusing on user-friendliness, might overlook the specific issue of service explanation clarity. The direct comparison in A/B testing controls for extraneous variables, offering more solid evidence for your conversion problem solution."
...
hypothesis_prompt = "Given the business we are in, the problem that we are facing, and the KPI that we want to use to measure whether or not we're solving our problem, which hypothesis do you think is most accurate? Explain your reasoning (see below). The person seeing this message will not have access to the data sent to you, so please restate the hypothesis in a sentence and then give your response. Do NOT mention the hypothesis number"
example_expected_learning_response= "The Two-Step Form Test, designed to improve conversion, replaces the current form with a two-tier process, first screening for ideal customers and then gathering contact details. This test directly validates or invalidates our hypothesis by assessing the effectiveness of enhancing lead qualification."
kpi_prompt = "Select the KPI that makes the most sense to measure our problem with. This should be one sentence. Then, in a new line, pick three other KPIs and briefly describe why they wouldn't work as well or shouldn't be the primary KPI: "
test_selection_prompt = "Pick a test that you recommend, and write a 1-paragraph explanation as to how you think it will help to validate/invalidate the hypothesis. In that paragraph, briefly describe why this test works better than two of the other alternates. Do not include a description for how to perform the test, or any information other than what I've asked for."
